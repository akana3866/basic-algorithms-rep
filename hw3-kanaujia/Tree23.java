...
class Tree23 extends Util {
	...
	LeafNode search (String x){
		...
		int i = u.findChild(x);
		LeafNode v = (LeafNode)u.child[i];
		if (v.item().key.equals(x)) 
			return v;
	...
	boolean insert (Item it){
		...
		int i = u.findChild(it.key);
		LeafNode v = (LeafNode)u.child[i];
		if (v.item().key.equals(it.key)) 
			return false; // it.key is already in the tree!
		u.addLeaf(it);
	...
	InternalNode find(String x) {
		...
		while (true) {
			int i;
			for (i = 0; i < u.degree() - 1; i++) {
				if (x.compareTo(u.child[i].guide) < 0)
					break;
			}
	
			if (u.child[i] instanceof LeafNode)
				break;
	...
	LeafNode search(String x) {
		...
		for (int i = 0; i < u.degree() - 1; i++) {
			if (u.child[i] instanceof LeafNode) {
				LeafNode leaf = (LeafNode) u.child[i];
				if (leaf.item().key.equals(x))
					return leaf;
			}
		}
	...
	boolean insert(Item it) {
        ...
        int result = u.addLeaf(it);

        if (result >= 2) {
            handleExceedingDegree(u);
            updateGuidesAfterInsert(root, it);
            return true;
        }
    ...
    void handleExceedingDegree(InternalNode u) {
        while (u.degree() == 4) {
            InternalNode newNode = new InternalNode(u.child[0], u.child[1], u.parent);
            newNode.parent = u.parent;
            u.child[0] = u.child[2];
            u.child[1] = u.child[3];
            u.child[2] = null;
            u.child[3] = null;
            ...
            if (u.parent == null) {
                root = new InternalNode(newNode, u, u.parent);
                ht++;
            } else {
                int insertIndex = findInsertIndex(u.parent, newNode);
                for (int j = u.parent.degree(); j > insertIndex; j--) {
                    u.parent.child[j] = u.parent.child[j - 1];
                }
                u.parent.child[insertIndex] = newNode;
            }
    ...
    int findInsertIndex(InternalNode parent, InternalNode newNode) {
        for (int i = 0; i < parent.degree(); i++) {
            if (parent.child[i].guide.compareTo(newNode.guide) > 0) {
                return i;
            }
        }
    ...
    void updateGuidesAfterInsert(InternalNode node, Item it) {
        if (node.guide.compareTo(it.key) < 0) {
            InternalNode k = find(it.key);
            while (k != null) {
                k.guide = it.key;
                k = k.parent;
            }
	...
	Item delete(String x) {
        InternalNode u = find(x); // never null
        Item deleted = u.removeLeaf(x);

        if (deleted == null) return null;

        while (u != null && u.guide.compareTo(u.child[u.degree() - 1].guide) != 0) {
            u.guide = u.child[u.degree() - 1].guide;
            u = u.parent;
        }

        if (u == null || u.degree() == 2 || u.parent == null) {
            return deleted;
        }

        int i = findChildIndex(u.parent, u);
        InternalNode sibling = getSibling(u.parent, i);

        if (sibling.degree() == 2) {
            if (i > 0) {
                sibling.proposeLeft(i);
            } else {
                sibling.proposeRight(i);
            }

            return handleDegreeOne(u, deleted);
        } else {
            if (i > 0) {
                splitLeft(u, sibling, i);
            } else {
                splitRight(u, sibling, i);
            }
        }

        return handleDegreeOne(u, deleted);
    }
	Item handleDegreeOne(InternalNode u, Item deleted) {
		while (u.parent != null && u.parent.degree() == 1) {
			u = u.parent;
			if (u.parent == null) {
			...
			int j = findChildIndex(u.parent, u);
	
			InternalNode newSibling = (j > 0) ? (InternalNode) u.parent.child[j - 1] : (InternalNode) u.parent.child[1];
	
			if (newSibling.degree() == 2) {
				newSibling.proposeLeft(j);
			} else {
				if (j > 0) {
					splitLeft(u, newSibling, j);
				} else {
					splitRight(u, newSibling, j);
				}
			}
		}
	...
	void splitLeft(InternalNode u, InternalNode leftSib, int i) {
		u.child[1] = u.child[0];
		u.child[0] = leftSib.child[2];
		leftSib.child[2]=null;
		u.child[0].parent = u;
		leftSib.guide = leftSib.child[1].guide;
		u.guide = u.child[1].guide;
		if (i==u.parent.degree()-1){
			u.parent.guide = u.guide;
		}
	}

	void splitRight(InternalNode u, InternalNode rightSib, int i){
		u.child[1] = rightSib.child[0];
		u.child[1].parent = u;
		u.guide = u.child[1].guide;
		rightSib.shiftLeft(0);
	}
	
	int findChildIndex(InternalNode parent, InternalNode child) {
        for (int i = 0; i < parent.degree(); i++) {
    ...
    InternalNode getSibling(InternalNode parent, int index) {
        if (index > 0) {
            return (InternalNode) parent.child[index - 1];
        } else {
            return (InternalNode) parent.child[1];
        }
    }
