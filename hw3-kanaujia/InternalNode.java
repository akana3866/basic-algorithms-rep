...
class InternalNode extends Node {
	...
	Item removeLeaf (String x){
		...
		void mergeChild(int index) {
			assert(index >= 0 && index < 3 && child[index] != null && child[index + 1] != null);
	
			InternalNode leftChild = (InternalNode) child[index];
			InternalNode rightChild = (InternalNode) child[index + 1];
	
			// Move the guide from the parent to the left child
			leftChild.guide = guide;
	
			// Move keys and children from the right child to the left child
			for (int i = 0; i < rightChild.childrenCount(); i++) {
				leftChild.addChild(rightChild.getChild(i));
				if (rightChild.getChild(i) != null) {
					rightChild.getChild(i).parent = leftChild;
				}
			}
	
			// Remove the separator key and child[index + 1] from the parent
		...
		InternalNode split() {
			...
			for (int i = 2; i < 4; i++) {
				newRightNode.addChild(child[i]);
				if (child[i] != null) {
					child[i].parent = newRightNode;
					child[i] = null;
				}
			}
	
			// Update the guide for the current node
			guide = child[1].guide;
