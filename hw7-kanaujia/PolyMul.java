...
public class PolyMul extends Util {
    ...
    // Ensure w is a primitive root of unity
    if (w_n != 1) {
        throw new IllegalArgumentException("w must be a primitive n'th root of unity modulo p");
    }
    ...
    long[] y0 = FFT(a0, wsq, p);
    long[] y1 = FFT(a1, wsq, p);

    long[] y = new long[n];
    long w_i = 1;
    for (int i = 0; i < n / 2; i++) {
        long y0_i = y0[i];
        long y1_i = (w_i * y1[i]) % p;
        y[i] = (y0_i + y1_i) % p;
        y[i + n / 2] = (y0_i - y1_i + p) % p; // Ensure the subtraction doesn't go negative
        w_i = (w_i * w) % p;
    }
    return y;
}
